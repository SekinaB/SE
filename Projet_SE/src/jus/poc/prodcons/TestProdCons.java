...
public class TestProdCons extends Simulateur {
	...
	static Observateur observateur = new Observateur();
	MyObservateur myobservateur;
	
	public TestProdCons(MyObservateur myobservateur) {
		super(observateur);
	...
	protected void run() throws Exception {
		...
		myobservateur.init(nbProd, nbCons, nbBuffer);

		// Creation du buffer
		ProdCons buffer = new ProdCons(nbBuffer, nbProd, myobservateur);
		...
		for (int i = 0; i < nbProd; i++) {
			...
			Producteur currentProd = new Producteur(buffer, i, nombreDeProduction, myobservateur, tempsMoyenProduction,
					deviationTempsMoyenProduction);
			//
			myobservateur.newProducteur(currentProd);
			if (FLAG_DEBUG) {
		...
		for (int i = 0; i < nbCons; i++) {
			Consommateur currentCons = new Consommateur(buffer, i, myobservateur, tempsMoyenConsommation,
					deviationTempsMoyenConsommation);
			//
			myobservateur.newConsommateur(currentCons);
	...
	public static void main(String[] args) {
		new TestProdCons(new MyObservateur()).start();
	}
